...
#Creation d'index
    ...
    if  'index_name_date_room' in collection.index_information():
        return
    else:
        try:
           ...
           if  "not authorized" in  e.details.get("errmsg").lower():      
                print("Vous n'avez pas les droits pour creer un index.")
           else:
                print("Erreur mongo :" , e.details.get("errmsg") )
    ...
    if  'index_name' in collection.index_information():
        ...
        try:
            collection.create_index([("Name", 1)],name="index_name") # Creation de l'index
        except Exception as e:
           if  "not authorized" in  e.details.get("errmsg").lower():      
                print("Vous n'avez pas les droits pour creer un index.")
           else:
